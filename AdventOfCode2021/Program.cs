using AdventOfCode2021.Day1;
using AdventOfCode2021.Day2;
using AdventOfCode2021.Day3;
using AdventOfCode2021.Day4;
using AdventOfCode2021.Day5;
using AdventOfCode2021.Day6;

Console.WriteLine($"Day 1 - Part 1: {Day1.GetSum(1)}");
Console.WriteLine($"Day 1 - Part 2: {Day1.GetSum(3)}");

Console.WriteLine($"Day 2 - Part 1: {Day2.PartOne()}");
Console.WriteLine($"Day 2 - Part 2: {Day2.PartTwo()}");

Console.WriteLine($"Day 3 - Part 1: {Day3.PartOne()}");
Console.WriteLine($"Day 3 - Part 2: {Day3.PartTwo()}");

Console.WriteLine($"Day 4 - Part 1: {Day4.PartOne()}");
Console.WriteLine($"Day 4 - Part 2: {Day4.PartTwo()}");

Console.WriteLine($"Day 5 - Part 1: {Day5.PartOne()}");

Console.WriteLine($"Day 6 - Part 1: {Day6.PartOne()}");
Console.WriteLine($"Day 6 - Part 2: {Day6.PartTwo()}");

Console.ReadLine();